...
csp_conn_t * csp_conn_allocate(csp_conn_type_t type) {
	...
	if (conn && (conn->state == CONN_CLOSED)) {
		...
		//QueueHandle_t xQueue=conn->rx_queue[0];
		//conn->rx_queue[0] =NULL;
		//csp_queue_remove(conn->rx_queue[0]);
		//conn->rx_queue[0] = csp_queue_create(csp_conf.conn_queue_length, sizeof(csp_packet_t *));
		//xQueue=conn->rx_queue[0];
