...
int csp_can_rx(csp_iface_t *iface, uint32_t id, const uint8_t *data, uint8_t dlc, CSP_BASE_TYPE *task_woken)
{
	...
	case CFP_MORE:
		...
		if (buf->rx_count == buf->packet->length-8) {
		  uint8_t dt[8];
          memcpy(dt, data,8);
          int a=0;
		}
		...
		/* Data is available */
		csp_qfifo_write(buf->packet, iface, 0);

		/* Drop packet buffer reference */
		buf->packet = NULL;

		/* Free packet buffer */
		csp_can_pbuf_free(buf, 0);
...
int csp_can_rx_fifo (csp_iface_t *iface, can_frame_t frame) {

	fs_rx_fifo.msgs[fs_rx_fifo.head].id      = frame.id;
	fs_rx_fifo.msgs[fs_rx_fifo.head].data_sz = frame.dlc;
	memcpy((void*)(fs_rx_fifo.msgs[fs_rx_fifo.head].data), (void*)frame.data, DRV_CSP_CAN_BUFFER_SZ);

	if (!fs_rx_fifo.full && NULL != fs_rx_fifo.msgs)
	{
		fs_rx_fifo.head++;

		if (fs_rx_fifo.head >= fs_rx_fifo.size)
		{
			fs_rx_fifo.last_frame = fs_rx_fifo.head;
			fs_rx_fifo.head = 0;
		}

		if (fs_rx_fifo.head == fs_rx_fifo.tail)
		{
			fs_rx_fifo.full = true;
		}


		if (FS_IS_LAST_MSG(frame.id))
		{
			//drv_csp_can_receive(fs_rx_fifo.msgs);
			int a=0;
		}
	}
...
int csp_can_add_interface(csp_iface_t * iface) {
	...
	//if ((iface->name == NULL) || (iface->interface_data == NULL)) {
	   return CSP_ERR_INVAL;
	}

    csp_can_interface_data_t * ifdata = iface->interface_data;
    ifdata->tx_func = drv_csp_can_transmit;
...
csp_can_interface_data_t ifdatX ={
    .cfp_frame_id = 0,
    .tx_func = drv_csp_can_transmit,
};
csp_iface_t csp_if_can = {
	.name = "CAN",
	.nexthop = csp_can_tx,
	.mtu = CSP_CAN_MTU,
	.interface_data = &ifdatX,
};
